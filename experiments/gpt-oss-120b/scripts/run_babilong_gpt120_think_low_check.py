...
def main():
    ...
    parser.add_argument('--model', default='gpt120:latest')
    parser.add_argument('--num-predict', type=int, default=24)
    parser.add_argument('--num-ctx', type=int, default=24576)
    parser.add_argument('--out-dir', default='/home/himura_shiro/Projects/BabiLong/experiments/gpt-oss-120b/results')
    parser.add_argument(
        '--run-id',
        default=None,
    ...
    run_tag = f'babilong_{args.task}_n{len(samples)}_{args.model}_think_low'
    ...
    with pred_path.open('a', encoding='utf-8') as wf:
        ...
        for s in samples:
            ...
            for attempt in range(args.max_retries + 1):
                attempts = attempt + 1
                try:
                    pred, thinking = gpt_chat_think_low(

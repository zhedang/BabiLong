...
def qwen_chat_think_fast(
    model: str, question: str, context: str, num_predict: int, num_ctx: int
) -> tuple[str, str | None]:
    # Allow thinking, but strongly constrain for speed and concise final output.
    response = chat(
        model=model,
        messages=[
            {
                'role': 'system',
                'content': (
                    'You may think briefly. Keep internal reasoning minimal and concise. '
                    'Then output only the final answer text from the context, no extra words.'
                ),
            },
            {
                'role': 'user',
...
def main():
    ...
    parser.add_argument('--out-dir', default='/home/himura_shiro/Projects/BabiLong/experiments/qwen3.5-9b/results')
    parser.add_argument(
        '--run-id',
        default=None,
    ...
    run_tag = f'babilong_{args.task}_n{len(samples)}_{args.model}_think_fast'
    ...
    with pred_path.open('a', encoding='utf-8') as wf:
        ...
        for s in samples:
            ...
            for attempt in range(args.max_retries + 1):
                attempts = attempt + 1
                try:
                    pred, thinking = qwen_chat_think_fast(
